...
Serial.println("LABEL, Time, Voltage, TempCcold, TempChot");//Name labels for columns in Excel
...
void loop(){
  ...
  int sum = 0;
  for (int i=0; i<30; i++) {
  sum=sum+analogRead(A0); //for loop to sum 30 measurments
  }
  ...
  float tempcold=analogRead(3); //Readiag data from the sensor. Stored as a 10bit number
  float TempCcold=(5*tempcold*1000.0)/(1024*10);
  float temphot=analogRead(2);
  float TempChot=(5*tempcold*1000.0)/(1024*10);
  // print out the value you read:
  Serial.print(Time); //print time elapsed
  Serial.print(",");
  Serial.print(Vs,5);
  Serial.print(",");
  Serial.print(voltageR2,5);
  Serial.print(",");
  Serial.print(TempCcold);
  Serial.print(",");
  Serial.println(TempChot);//print total voltage with 5 decimals
