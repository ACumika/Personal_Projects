...
def predict_image(img):
    ...
    def plot_cont(mask_clean):
        ...
        contours = cv.findContours(mask_int, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
        return contours
    ...
    def trains_cord(contours, img_cnt):
        trains = []
    ...
    def trains(img):
        ...
        img_cnt = img.copy()
        red_trains = trains_cord(red_cont, img_cnt)
        blue_trains = trains_cord(blue_cont, img_cnt)
        black_trains = trains_cord(black_cont, img_cnt)
        green_trains = trains_cord(green_cont, img_cnt)
        yellow_trains = trains_cord(yellow_cont, img_cnt)
